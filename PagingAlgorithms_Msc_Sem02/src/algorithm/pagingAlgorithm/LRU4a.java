 ...
 * Cache simulated via Int[] für direkte Addressierung und Int[] für cache
 * 
 * 
 * @author Markus Strobel, Justine Smyzek
 */
public class LRU4a extends Algorithm {
	 ...
	 * @param pageRange
	 */
	public LRU4a(String name, int cacheSize, int pageRange)
	{
		this._pageFaults = 0;
	...
	public void processNextPage(int page) {		
		...
		else
		{
			...
			for(int _cachePosition = 0; _cachePosition < _cache.length; _cachePosition++)
			{
				...
				if(_cache[_cachePosition] > -1)
				{
					// get the direct address of the _caches page in the address space from the _cache[_cachePosition]
					if(_addressSpace[_cache[_cachePosition]] < min)
					{
						if(_cache[_cachePosition] > -1)
							min = _addressSpace[_cache[_cachePosition]];
						oldPageAddressSpacePos = _cache[_cachePosition];
	...
	public void reset() {
		...
		this._cache = new int[this._cacheSize];
		for(int i = 0; i < _cacheSize; i++)
		{
			this._cache[i] = -1;
		}	
	...
	public Algorithm create(String name, int cacheSize, int pageRange, int randomRange) {
		return new LRU4a(name, cacheSize, pageRange);
	}
}
